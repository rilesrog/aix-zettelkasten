...
a a a a
Raoul Schönhof\*, Artem Werner, Jannes Elstner, Boldizsar Zopcsak

## Ramez Awad a , Marco Huber a

## a a Ramez Awad, Marco Huber

a _Fraunhofer Institute for Manufacturing Engineering and Automation IPA, Nobelstraße 12, Stuttgart, Germany_

- Corresponding author. Tel.: +49-711-970-1843; _E-mail address:_ [raoul.schoenhof@ipa.fraunhofer.de](mailto:raoul.schoenhof@ipa.fraunhofer.de)
...
_Keywords:_ Assembly Automation, Deep Learning, xAI, , NeuroCAD, Grad-CAM, LIME, LRP
...
For the formal definition, the relevance or importance 𝑅𝑖
of an input 𝑥 is defined as \[10\].

$$
R\_{i}(x)=\\left(\\frac{\\partial f}{\\partial x\_{i}}\\right)^{2}
$$
...
Defining 𝑧𝑗𝑘= 𝑎𝑗𝑤𝑗𝑘as the weighted activation of
...
$$
z\_{j k}=a\_{j}w\_{j k}
$$

$$
R \_ {j} ^ {(l)} = \\sum\_ {k} \\left(\\frac {z \_ {j k}}{\\sum\_ {j} z \_ {j k}} R \_ {k} ^ {(l + 1)}\\right)
$$
...
weight 𝑤𝑗𝑘, the weight has to be interpreted as the gradient
𝜕𝑎𝑘
.
𝜕𝑎𝑗

$$
R \_ {j} ^ {(l)} = \\sum\_ {k} \\left(\\frac {z \_ {j k}}{\\sum\_ {j} z \_ {j k} + \\epsilon s i g n (\\sum\_ {j} z \_ {j k})} R \_ {k} ^ {(l + 1)}\\right)
$$
...
$$
\\frac{\\partial a\_{k}}{\\partial a\_{j}}
$$
...
$$
\\sum\_{j}\\left(,R\_{j}^{;(l+1)}\\right),=,\\sum\_{k}\\left(,R\_{k}^{;(l+1)}\\right)
$$
...
$$
\\zeta(x)=a r g m i n\\mathcal{L}(f,g,\\pi\_{x})+\\mathcal{I}(g)
$$

where 𝑥 is the input data being explained, 𝑔 is a statistical
method and 𝛺(𝑔) is the complexity of this statistical method.
The locality-aware loss function 𝓛(𝑓, 𝑔, 𝜋𝑥) is defined by:

$$
\\mathcal{L}(f,g,\\pi\_{x})
$$

$$
\\mathcal{L}(f,g,\\pi\_{x})={\\textstyle\\sum}\\pi\_{x}(z)(f(z)-g(z^{\\prime}))^{2}
$$

The proximity 𝜋𝑥between the original input and
perturbed data 𝑧 is defined by:

$$
\\pi\_ {x} (z) = \\exp \\left(- D (x, z) ^ {2} / \\sigma^ {2}\\right)
$$
...
$$
\\alpha\_{k}^{c}=\\frac{1}{Z}\ {sum{\\sum} _{i}}{{\\sum}_{j}}\\frac{{\\partial{y^{c}}}}{{\\partial{A\_{i j}^{k}}}}
$$
...
𝑘
gradient of the score for the class 𝑐 and 𝜕𝐴𝑖𝑗is the gradient
𝑘
...
$$
L^{c}=R e L U(\\sum\_{k}\\alpha\_{k}^{c}A^{k})
$$
...
(2000). [https://doi.org/10.1023/A:1008199714982](https://doi.org/10.1023/A:1008199714982).
